...
export function ScannerContainer() {
  ...
  // Trigger OCR initialization in the background on mount
  useEffect(() => {
    initOcr();
    return () => {
      if (ocrWorker) {
  ...
  const handleRunOcr = async (pageId: string) => {
    if (!ocrInitialized || !ocrWorker) {
      toast.error("OCR engine is still loading. Please wait.");
      return;
    }
    ...
    toast.loading("Running OCR text recognition...", { id: "ocr-toast" });
    ...
    try {
      ...
      const { data } = await ocrWorker.recognize(imageToOcr);
    ...
    <div className="min-h-[calc(100vh-120px)] bg-slate-900 rounded-3xl border border-slate-800 text-white flex flex-col font-sans overflow-hidden shadow-2xl relative">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="flex-1 bg-slate-950 flex flex-col items-center justify-center p-4 relative min-h-[350px]">
          {activeTab === "camera" ? (
            <div className="w-full h-full max-w-2xl aspect-[3/4] sm:aspect-[4/3] max-h-[500px] relative">
              <CameraPreview
                ref={cameraRef}
            ...
            <div className="p-3 border-t border-slate-800 bg-slate-950/20">
              {activePage.ocrText ? (
                ...
                <button
                  ...
                  disabled={ocrProcessingPageId === activePage.id || !ocrInitialized}
